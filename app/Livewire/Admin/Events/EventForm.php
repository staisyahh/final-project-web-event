...
class EventForm extends Form
{
    ...
    public function setEvent(\App\Models\Event $event)
    {
        ...
        $this->tickets = $event->tickets->map(fn ($ticket) => $ticket->only(['name', 'harga', 'stok']))->toArray();
        $this->selectedSpeakers = $event->speakers->pluck('id')->toArray();
        
        // Populate existing gallery
        $this->existingGallery = $event->galleries->pluck('image_url')->toArray();
        $this->newGallery = [];
    }
    ...
    {
        $this->tickets[] = ['name' => '', 'harga' => 0, 'stok' => 100];
    }
