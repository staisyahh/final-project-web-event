<div>
                ...
                <div class="card">
                    <div class="card-content">
                        <img src="{{ $registration->event->banner_url ?? 'https://via.placeholder.com/400x200.png?text=Event+Banner' }}"
                            class="card-img-top img-fluid" alt="event-banner" style="height: 200px; object-fit: cover;">
                        <div class="card-body">
                            <span class="badge {{
                                    [
                                        ...
                                        'cancelled' => 'bg-danger'
                                    ][$registration->status] ?? 'bg-secondary'
                                }}">
                                {{ str_replace('_', ' ', Str::title($registration->status)) }}
                            </span>

                            <h5 class="card-title mt-3">{{ $registration->event->title }}</h5>
                            <p class="card-text text-muted">
                                <i class="bi bi-calendar-check"></i> {{ $registration->event->jadwal->format('d M Y,
                                H:i') }} WIB<br>
                                <i class="bi bi-geo-alt"></i> {{ $registration->event->location_name }}
                            </p>
                        </div>
                    </div>

                    {{-- Tampilkan E-Tiket atau Tombol Aksi berdasarkan Status --}}
                    @if ($registration->status == 'confirmed' && $registration->eTickets->isNotEmpty())
                        {{-- TIKET SUKSES: Tampilkan E-Ticket --}}
                        @foreach ($registration->eTickets as $eTicket)
                        <div class="card-footer border-top">
                            <h6 class="text-center">E-Ticket #{{ $loop->iteration }}</h6>
                            <div class="text-center d-flex justify-content-center">
                                <div class="qrcode-container" data-code="{{ $eTicket->ticket_code }}"></div>
                            </div>
                            <p class="text-center text-muted mt-2 mb-0" style="letter-spacing: 2px;">
                                <strong>{{ $eTicket->ticket_code }}</strong>
                            </p>
                        </div>
                        @endforeach

                        {{-- SEBAGAI TAMBAHAN: Jika event selesai, tampilkan opsi review --}}
    ...
    <x-ui.modal name="upload-proof-modal" title="Unggah Bukti Pembayaran">
        <form wire:submit="savePaymentProof" x-data="{ uploading: false, progress: 0 }"
            ...
            <div class="mb-3">
                ...
                <input type="file" wire:model="paymentProofFile" id="paymentProofFile"
                    class="form-control @error('paymentProofFile') is-invalid @enderror">
            ...
            <div class="alert alert-info small" role="alert">
                Pastikan bukti pembayaran jelas dan berisi informasi transaksi (bank, tanggal, jumlah). Ukuran maksimal 2MB.
            </div>

            <div class="d-grid mt-4">
                <x-ui.button type="submit" variant="primary" size="lg"
                    x-bind:disabled="uploading || {{ json_encode(!$paymentProofFile) }}">
    ...
    <x-ui.modal name="review-modal" title="Beri Ulasan">
        @if($selectedEventForReviewId)
            @livewire('member.submit-review-form', ['eventId' => $selectedEventForReviewId], key('review-form-'.$selectedEventForReviewId))
        @endif
    </x-ui.modal>
</div>

@push('scripts')
{{-- Load qrcode.js library from a reliable CDN --}}
...
<script>
    function generateQrCodes() {
        ...
        // If no elements found, do nothing
        if (qrCodeElements.length === 0) {
            return;
        }

        // Generate a QR code for each element
        qrCodeElements.forEach(el => {
            // Clear any previous QR code
            el.innerHTML = '';

            // Get the ticket code from the data attribute
            const ticketCode = el.dataset.code;
    ...
    // IMPORTANT: Listen for Livewire's 'navigated' event
    // This ensures the script runs on initial page load AND on subsequent SPA-like navigations
